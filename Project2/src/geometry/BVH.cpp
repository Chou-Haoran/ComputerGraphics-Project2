      ...
      primitives(std::move(p))
{
    ...
    int hrs  = static_cast<int>(diff) / 3600;
    int mins = (static_cast<int>(diff) / 60) - (hrs * 60);
    int secs = static_cast<int>(diff) - (hrs * 3600) - (mins * 60);
...
BVHBuildNode* BVHAccel::recursiveBuild(std::vector<Object*> objects, int dim)
{
    ...
    if (objects.size() == 1) {
        ...
        // Median-split along a cyclic axis (NAIVE). TODO: implement SAH for
        // a better split, which can pay off on heavy meshes (≥100k tris).
        switch (dim % 3) {
            case 0:
...
Intersection BVHAccel::getIntersection(BVHBuildNode* node, const Ray& ray) const
{
    Intersection inter;
    Vector3f invDir(
        1.0f / (ray.direction[0] == 0 ? 0.00001f : ray.direction[0]),
        1.0f / (ray.direction[1] == 0 ? 0.00001f : ray.direction[1]),
        1.0f / (ray.direction[2] == 0 ? 0.00001f : ray.direction[2]));
